...
const BottomCardCarousel = () => {
  ...
  const fetchImages = () => {
    fetch(`${url}/api/admin/get/bottom-card-carousel`)
      .then((response) => {
        if (!response.ok) {
  ...
  const handleUpload = () => {
    ...
    fetch(`${url}/api/admin/upload/bottom-card-carousel`, {
      method: 'POST',
      body: formData,
    })
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await fetch(`${url}/api/admin/delete-one/bottom-card-carousel`, {
        method: 'DELETE',
      });
      if (!response.ok) {
...
export default BottomCardCarousel;
